...
data layer). No central server — each user's data lives in their own AT Protocol
repository, synced in real time via Jetstream.
...
with collaborators. Joining a board fetches it from the owner's PDS (Personal
Data Server) and subscribes to real-time updates through [Jetstream], Bluesky's
WebSocket event stream.

[Jetstream]: https://docs.bsky.app/blog/jetstream

All data is stored locally in IndexedDB (via Dexie) and synced to each user's
PDS in the background. The app works offline and catches up when reconnected.

## Architecture

Each user runs the full stack in their browser. There is no central server —
data syncs through AT Protocol personal data servers, with real-time updates
relayed via Jetstream.
...
The board owner's PDS is the source of truth for board configuration and trust
grants. Ops in any PDS can reference tasks in any other PDS via AT URI.
...
On reconnect or stale cursor (>48h offline), the app backfills by fetching
directly from all known participants' PDS endpoints.
...
[fractional-indexing]: https://www.npmjs.com/package/fractional-indexing
[@atproto/api]: https://www.npmjs.com/package/fractional-indexing
...
The CLI authenticates via OAuth and talks directly to PDS endpoints — no local
database. Each command fetches fresh data from the board owner's PDS and all
trusted participants, runs the same `materializeTasks()` merge logic as the web
app, and displays the result. Write commands (`new`, `mv`, `edit`, `comment`)
create AT Protocol records (tasks, ops, comments) in your PDS, which the web app
picks up in real time via Jetstream.
