...
async function fetchParticipantData(
  ...
  // PlacementOps: boardUri may be absent on new records, derive from target placement
  await fetchAndStorePlacementOps(did, boardUri);
...
async function fetchAndStorePlacementOps(
  did: string,
  boardUri: string,
): Promise<void> {
  const records = await fetchRecordsFromRepo(did, PLACEMENT_OP_COLLECTION);
  for (const record of records) {
    const validated = safeParse(
      PlacementOpRecordSchema,
      record.value,
    ...
    // Derive boardUri: use record's boardUri if present, else look up target placement
    let derivedBoardUri = validated.boardUri;
    if (!derivedBoardUri) {
      const placement = getPlacementByUri(validated.targetPlacementUri);
      derivedBoardUri = placement?.boardUri;
    }
    // Only store if this op belongs to the board we're backfilling
    if (derivedBoardUri !== boardUri) continue;

    const rkey = record.uri.split("/").pop()!;
    upsertPlacementOp(did, rkey, {
      targetPlacementUri: validated.targetPlacementUri,
      boardUri: derivedBoardUri,
      fields: validated.fields,
      createdAt: validated.createdAt,
    });
  }
