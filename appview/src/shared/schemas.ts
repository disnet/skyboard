...
export const BoardRecordSchema = z.object({
  ...
  labelUris: z.array(z.string().max(MAX_STRING)).max(MAX_LABELS).optional(),
  open: z.boolean().optional(),
  createdAt: z.string().max(MAX_STRING),
});

const BoardOpFieldsSchema = z.object({
  name: z.string().max(MAX_STRING).optional(),
  description: z.string().max(MAX_STRING).optional(),
  columns: z.array(ColumnSchema).max(MAX_COLUMNS).optional(),
  labelUris: z.array(z.string().max(MAX_STRING)).max(MAX_LABELS).optional(),
  open: z.boolean().optional(),
});

export const BoardOpRecordSchema = z.object({
  targetBoardUri: z.string().max(MAX_STRING),
  fields: BoardOpFieldsSchema,
  createdAt: z.string().max(MAX_STRING),
});
...
export const TaskRecordSchema = z.object({
  ...
  labelUris: z.array(z.string().max(MAX_STRING)).max(MAX_LABEL_IDS).optional(),
...
export const PlacementOpRecordSchema = z.object({
  ...
  boardUri: z.string().max(MAX_STRING).optional(), // Deprecated: derived from target placement
...
const ProjectOpFieldsSchema = z.object({
  name: z.string().max(MAX_STRING).optional(),
  description: z.string().max(MAX_STRING).optional(),
  labelUris: z.array(z.string().max(MAX_STRING)).max(MAX_LABELS).optional(),
});

export const ProjectOpRecordSchema = z.object({
  targetProjectUri: z.string().max(MAX_STRING),
  fields: ProjectOpFieldsSchema,
  createdAt: z.string().max(MAX_STRING),
});

const CommentOpFieldsSchema = z.object({
  text: z.string().max(MAX_STRING).optional(),
});

export const CommentOpRecordSchema = z.object({
  targetCommentUri: z.string().max(MAX_STRING),
  fields: CommentOpFieldsSchema,
  createdAt: z.string().max(MAX_STRING),
});
