...
export function upsertBoardOp(
  did: string,
  rkey: string,
  record: { targetBoardUri: string; fields: unknown; createdAt: string },
): void {
  const db = getDb();
  const uri = buildAtUri(did, "dev.skyboard.boardOp", rkey);
  db.run(
    `INSERT INTO board_ops (uri, did, rkey, targetBoardUri, fields, createdAt)
     VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT(uri) DO UPDATE SET
       targetBoardUri=excluded.targetBoardUri,
       fields=excluded.fields, createdAt=excluded.createdAt`,
    [
      uri,
      did,
      rkey,
      record.targetBoardUri,
...
export function getBoardOpsByBoard(boardUri: string): BoardOpRow[] {
  const db = getDb();
  return db
    .query<
      BoardOpRow,
      [string]
    >("SELECT * FROM board_ops WHERE targetBoardUri = ?")
    .all(boardUri);
}

export function deleteBoardOp(did: string, rkey: string): void {
  const db = getDb();
  const uri = buildAtUri(did, "dev.skyboard.boardOp", rkey);
  db.run("DELETE FROM board_ops WHERE uri = ?", [uri]);
}
...
export function getApprovalsByTargetUris(targetUris: string[]): ApprovalRow[] {
  if (targetUris.length === 0) return [];
  const db = getDb();
  const placeholders = targetUris.map(() => "?").join(",");
  return db
    .query<
      ApprovalRow,
      string[]
    >(`SELECT * FROM approvals WHERE targetUri IN (${placeholders})`)
    .all(...targetUris);
}
...
export function getPlacementByUri(placementUri: string): PlacementRow | null {
  const db = getDb();
  return db
    .query<PlacementRow, [string]>("SELECT * FROM placements WHERE uri = ?")
    .get(placementUri);
}
...
export function upsertProject(
  ...
  db.run(
    `INSERT INTO projects (uri, did, rkey, name, description, labels, labelUris, createdAt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(uri) DO UPDATE SET
       name=excluded.name, description=excluded.description,
       labels=excluded.labels, labelUris=excluded.labelUris,
       createdAt=excluded.createdAt`,
    [
      uri,
      did,
      ...
      record.description ?? null,
      JSON.stringify(record.labels ?? []),
      JSON.stringify(record.labelUris ?? []),
      record.createdAt,
    ],
  );
}
...
export function upsertProjectOp(
  did: string,
  rkey: string,
  record: { targetProjectUri: string; fields: unknown; createdAt: string },
): void {
  const db = getDb();
  const uri = buildAtUri(did, "dev.skyboard.projectOp", rkey);
  db.run(
    `INSERT INTO project_ops (uri, did, rkey, targetProjectUri, fields, createdAt)
     VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT(uri) DO UPDATE SET
       targetProjectUri=excluded.targetProjectUri,
       fields=excluded.fields, createdAt=excluded.createdAt`,
    [
      uri,
      did,
      rkey,
      record.targetProjectUri,
      JSON.stringify(record.fields),
      record.createdAt,
    ],
  );
...
export function getProjectOpsByProject(projectUri: string): ProjectOpRow[] {
  const db = getDb();
  return db
    .query<
      ProjectOpRow,
      [string]
    >("SELECT * FROM project_ops WHERE targetProjectUri = ?")
    .all(projectUri);
}

export function deleteProjectOp(did: string, rkey: string): void {
  const db = getDb();
  const uri = buildAtUri(did, "dev.skyboard.projectOp", rkey);
  db.run("DELETE FROM project_ops WHERE uri = ?", [uri]);
}

// --- CommentOp ---

export interface CommentOpRow {
  uri: string;
  did: string;
  rkey: string;
...
export function upsertCommentOp(
  did: string,
  rkey: string,
  record: { targetCommentUri: string; fields: unknown; createdAt: string },
): void {
  const db = getDb();
  const uri = buildAtUri(did, "dev.skyboard.commentOp", rkey);
  db.run(
    `INSERT INTO comment_ops (uri, did, rkey, targetCommentUri, fields, createdAt)
     VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT(uri) DO UPDATE SET
       targetCommentUri=excluded.targetCommentUri,
       fields=excluded.fields, createdAt=excluded.createdAt`,
    [
      uri,
      did,
      rkey,
      record.targetCommentUri,
      JSON.stringify(record.fields),
      record.createdAt,
    ],
  );
...
export function getCommentOpsByTaskUris(taskUris: string[]): CommentOpRow[] {
  if (taskUris.length === 0) return [];
  const db = getDb();
  // Join through comments to find comment ops for tasks on this board
  const placeholders = taskUris.map(() => "?").join(",");
  return db
    .query<CommentOpRow, string[]>(
      `SELECT co.* FROM comment_ops co
       JOIN comments c ON co.targetCommentUri = c.uri
       WHERE c.targetTaskUri IN (${placeholders})`,
    )
    .all(...taskUris);
}

export function deleteCommentOp(did: string, rkey: string): void {
  const db = getDb();
  const uri = buildAtUri(did, "dev.skyboard.commentOp", rkey);
  db.run("DELETE FROM comment_ops WHERE uri = ?", [uri]);
}
...
export function upsertLabel(
  ...
  const db = getDb();
  const uri = buildAtUri(did, "dev.skyboard.label", rkey);
  db.run(
    `INSERT INTO labels (uri, did, rkey, name, color, description, createdAt)
     VALUES (?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(uri) DO UPDATE SET
       name=excluded.name, color=excluded.color,
       description=excluded.description, createdAt=excluded.createdAt`,
    [
      uri,
      did,
      rkey,
...
export function getLabelsByUris(labelUris: string[]): LabelRow[] {
  if (labelUris.length === 0) return [];
  const db = getDb();
  const placeholders = labelUris.map(() => "?").join(",");
  return db
    .query<
      LabelRow,
      string[]
    >(`SELECT * FROM labels WHERE uri IN (${placeholders})`)
    .all(...labelUris);
}

export function deleteLabel(did: string, rkey: string): void {
  const db = getDb();
  const uri = buildAtUri(did, "dev.skyboard.label", rkey);
  db.run("DELETE FROM labels WHERE uri = ?", [uri]);
}
