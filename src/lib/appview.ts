  ...
  try {
    ...
    await db.transaction(
      ...
      async () => {
        ...
        // Store board ops
        for (const bo of data.boardOps ?? []) {
          const boardOpData: Omit<BoardOp, "id"> = {
            rkey: bo.rkey,
            did: bo.did,
          ...
          const existing = await db.boardOps
            .where("[did+rkey]")
            .equals([bo.did, bo.rkey])
            .first();
          if (existing?.id) {
            if (existing.syncStatus === "pending") continue;
            if (
              !recordsEqual(existing, boardOpData, [
                "targetBoardUri",
                "fields",
                "createdAt",
              ])
            ) {
              await db.boardOps.update(existing.id, boardOpData);
            }
          } else {
            await db.boardOps.add(boardOpData as BoardOp);
          }
        }
      },
    );
